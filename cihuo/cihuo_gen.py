...
def show_tables(db_name, table_name):
    ...
    cursor.execute('SELECT * FROM ' + table_name + ' WHERE error_mark=? AND remembered=?',(0,1))
...
#cihuo check
def cihuo_gen(file_name):
    number = 0
    array = []
    for line in open (file_name):
        #arr = preg_splite('|',line)
        arr = line.encode("utf-8").split('|')
        array.append((number, arr[2]+arr[1], arr[0], unicode(''), 0, 1))
        number = number + 1
    return array
...
if __name__ == '__main__':
    ...
    file_name = "gw_3611.txt"
    array = cihuo_gen(file_name)
    table_name = file_name[:-4]
    #print("test")
    #creat_database("listentothis.db",table_name,array)
